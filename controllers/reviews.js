...
const reviewRouteParametersCheckSchema = {
...
async function create(req, res) {
  ...
  try {
    ...
    await checkSchema(reviewRouteParametersCheckSchema).run(req);
    ...
    if (!routeParametersErrors.isEmpty()) {
        ...
        if(errors_mapped.res_id || errors_mapped.item_id){
...
async function update(req, res) {
    ...
    try {
      ...
      if (!routeParametersErrors.isEmpty()) {
          const errors_mapped = routeParametersErrors.mapped(); 
          // check if one of the params IDs return an error
          if(errors_mapped.res_id || errors_mapped.item_id || errors_mapped.id ){
            req.flash('error', 'dont play with me please');
            return res.redirect(301, `/`);
          }
      }
